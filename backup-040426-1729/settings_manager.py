...
def show_timezone_settings_window(parent_root, update_main_timezone_display_callback, open_toplevel_windows_list=None):
    ...
    if open_toplevel_windows_list is not None:
        open_toplevel_windows_list.append(settings_win) # <<< اضافه شده

    # تابع برای حذف پنجره از لیست هنگام بسته شدن
    def on_settings_win_close():
        if open_toplevel_windows_list is not None and settings_win in open_toplevel_windows_list:
            open_toplevel_windows_list.remove(settings_win) # <<< اضافه شده
        settings_win.destroy()

    settings_win.protocol("WM_DELETE_WINDOW", on_settings_win_close) # <<< اضافه شده: هندل کردن دکمه بستن
